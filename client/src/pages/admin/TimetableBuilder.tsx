...
export default function TimetableBuilder() {
  ...
  const handlePrint = () => {
    ...
    if (!w) return
    w.document.write(`
      <html><head><title>Timetable - ${className}</title>
      <style>
        body { font-family: Arial, sans-serif; padding: 30px; max-width: 900px; margin: auto; }
        .header { display: flex; align-items: center; justify-content: center; gap: 12px; border-bottom: 3px solid #1e3a5f; padding-bottom: 14px; margin-bottom: 18px; text-align: center; }
        ...
        td.time { background: #f3f4f6; font-weight: bold; width: 70px; }
        .slot { font-size: 11px; }
        .slot .subject { font-weight: bold; color: #1e3a5f; }
        ...
        <div class="header">
          ...
          <div>
            ...
            <div class="sub">Weekly Timetable &mdash; ${className}</div>
          </div>
        </div>
        <table>
          <thead>
            <tr><th>Time</th>${DAYS.slice(0, 6).map((d) => `<th>${d}</th>`).join('')}</tr>
          </thead>
          <tbody>
            ${TIME_SLOTS.map((time) => {
              const rows = DAYS.slice(0, 6).map((day) => {
                ...
                const teacherName = slot.teacher?.user?.name || slot.teacher?.name || ''
                return `<td><div class="slot"><div class="subject">${slot.subject?.name || ''}</div><div>${teacherName}</div>${slot.room ? `<div>Room ${slot.room}</div>` : ''}</div></td>`
              }).join('')
              return `<tr><td class="time">${time}</td>${rows}</tr>`
            }).join('')}
          </tbody>
        </table>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={handlePrint} disabled={!selectedClass}>
            <Printer className="h-4 w-4 mr-2" /> Print Timetable
          </Button>
